...
export default function AdminCatalogPage() {
  ...
  const open = (row: CatalogRow | "new") => {
    ...
    if (meta.colorField) base.color = "#c93239";
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat']">Catálogos</h1>
        ...
        {TABS.map((x) => (
          <button
            ...
            className={cx(
              ...
              tab === x.id
                ? "bg-[#c93239] text-white border-[#c93239]"
                : "bg-white dark:bg-transparent text-gray-600 dark:text-white/60 border-gray-200 dark:border-white/20 hover:border-[#c93239]"
            )}
          >
            {x.label}
          </button>
          ...
          <Table head={["Slug", "Nome (PT)", "Nome (EN)", ...(meta.colorField ? ["Cor"] : []), "Ordem", "Ativo", "Ações"]}>
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-gray-50/60 dark:hover:bg-white/5">
                ...
                <td className="py-2.5 pr-4 font-medium text-[#c93239] dark:text-white">{row.name_pt}</td>
          ...
          <div className="space-y-4">
            ...
            {meta.colorField ? (
              <div>
                <Label>Cor (hex)</Label>
                <div className="flex gap-2">
                  <input type="color" className="h-10 w-14 rounded-lg border border-gray-200 dark:border-white/20 bg-transparent" value={/^#([0-9a-fA-F]{6})$/.test(String(form.color)) ? String(form.color) : "#c93239"} onChange={(e) => setForm({ ...form, color: e.target.value })} aria-label="Cor" />

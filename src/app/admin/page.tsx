...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat']">Dashboard</h1>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={s.label} className="bg-white dark:bg-[#c93239]/30 rounded-2xl border border-gray-100 dark:border-white/10 p-5 flex items-center gap-4">
            <span className={`w-11 h-11 rounded-xl bg-gray-50 dark:bg-white/10 flex items-center justify-center ${s.tone}`}>
              <s.icon size={20} aria-hidden="true" />
            </span>
            <span>
              <span className="block text-2xl font-bold font-['Montserrat'] text-[#c93239] dark:text-white">
                {data ? s.value : "—"}
              </span>
            ...
            <ul className="divide-y divide-gray-50 dark:divide-white/5">
              {data.updates.map((u) => (
                <li key={u.id} className="py-2.5 flex items-center justify-between gap-3">
                  <span className="text-sm text-[#c93239] dark:text-white line-clamp-1">{u.title_pt}</span>
        ...
        <Card title="Atalhos" actions={<Link className="text-xs text-[#E8821A] hover:underline" href="/admin/mensagens">Mensagens →</Link>}>
          <div className="grid grid-cols-2 gap-3 text-sm">
            {[
              ...
              <Link key={l.href} href={l.href} className="block border border-gray-100 dark:border-white/10 rounded-xl px-4 py-3 hover:border-[#E8821A] hover:bg-[#E8821A]/5 transition-colors text-[#c93239] dark:text-white">
                {l.label}
              </Link>
            ))}
          </div>

...
export default async function AboutPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a1628]">
      {/* Header */}
      <div className="bg-[#c93239] py-16 px-4 sm:px-6 text-center">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-14 space-y-16">
        ...
        {about ? (
          <section className="grid md:grid-cols-[280px_1fr] gap-10 items-start" aria-labelledby="msg-governador">
              ...
              <div>
                <div className="rounded-2xl overflow-hidden bg-gray-100 dark:bg-white/5 aspect-[3/4] relative">
                  {governorPhoto ? (
                    ...
                    <div className="w-full h-full flex items-end justify-center p-4 bg-gradient-to-b from-[#c93239]/80 to-[#c93239]">
                ...
                <div className="mt-4 text-center">
                  <p className="font-bold text-[#c93239] dark:text-white font-['Montserrat']">{about.governor_name}</p>
            ...
            <div>
              ...
              <h2 id="msg-governador" className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mt-2 mb-5">
        ...
        {(mission || vision || values.length > 0) && (
          <section>
            <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-8 text-center">{ui.about.missionVisionValues}</h2>
            <div className="grid sm:grid-cols-3 gap-6">
              {[
                ...
                .filter((c) => c.show)
                .map((item) => (
                  <div key={item.title} className="bg-[#F8F9FA] dark:bg-[#c93239]/30 rounded-2xl p-6 border border-gray-100 dark:border-white/10">
                    ...
                    <h3 className="font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-3">{item.title}</h3>
        ...
        {/* {org.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-8 text-center">{ui.about.orgTitle}</h2>
                ...
                <div key={o.id} className="flex flex-col items-center">
                  <div
                    className={`rounded-xl px-6 py-3 text-center border ${
                      i === 0
                        ? "bg-[#c93239] text-white border-[#c93239] min-w-[260px]"
                        : "bg-white dark:bg-[#c93239]/30 border-gray-200 dark:border-white/10 min-w-[240px]"
                    }`}
                  >
                    <div className={`text-xs font-medium mb-0.5 ${i === 0 ? "text-[#D4A843]" : "text-[#E8821A]"}`}>{bi(o, "role", locale)}</div>
                    <div className={`font-semibold text-sm font-['Montserrat'] ${i === 0 ? "text-white" : "text-[#c93239] dark:text-white"}`}>{o.name}</div>
                  </div>
        ...
        {/* {milestones.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-8">{ui.about.timelineTitle}</h2>
            <div className="space-y-0">
              {milestones.map((m, i) => (
                <div key={m.id} className="flex gap-6">
                  <div className="flex flex-col items-center" aria-hidden="true">
                    <div className="w-12 h-12 rounded-xl bg-[#c93239] flex items-center justify-center text-[#D4A843] font-bold text-xs font-['Montserrat'] shrink-0">
                      {m.year}
                    </div>
                    {i < milestones.length - 1 && <div className="w-0.5 flex-1 bg-gray-200 dark:bg-white/10 my-1" />}
                  </div>
                  <div className="pb-8">
                    <h3 className="font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-1">{bi(m, "title", locale)}</h3>
        ...
        {about?.video_url ? (
          <section>
            <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-6 text-center">{ui.about.videoTitle}</h2>
            <VideoEmbed
              url={about.video_url}
        ...
        {/* CTA */}
        <div className="text-center bg-[#F8F9FA] dark:bg-[#c93239]/20 rounded-2xl p-8">
          <h3 className="text-xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-3">{ui.about.ctaTitle}</h3>

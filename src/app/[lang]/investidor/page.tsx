...
function TestimonialPhoto({ t, locale }: { t: TestimonialOut; locale: Locale }) {
  ...
  if (!url) {
    return (
      <div className="w-10 h-10 rounded-full bg-[#c93239] dark:bg-white/10 flex items-center justify-center text-[#D4A843] text-xs font-bold shrink-0" aria-hidden="true">
        {initials(t.name)}
      </div>
    );
  }
      ...
      <div className="bg-[#c93239] py-10 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            {projectsRes.ok ? (
              <span className="bg-[#D4A843] text-[#c93239] text-sm font-bold px-3 py-1 rounded-full self-start sm:self-auto">
      ...
      {!projectsRes.ok ? (
        ...
        <div>
          <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            ...
            {indicators.length > 0 && (
              <section>
                <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-6">{ui.investor.indicatorsTitle}</h2>
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {indicators.map((ind) => (
                    <div key={ind.id} className="bg-[#F8F9FA] dark:bg-[#c93239]/30 rounded-xl p-5 border border-gray-100 dark:border-white/10">
                      <div className="mb-3">
                        <ApiIcon name={ind.icon} size={20} className={ind.color ? "" : "text-[#D4A843]"} />
                      </div>
                      <div className="text-xl font-bold text-[#c93239] dark:text-white font-['Montserrat']">{ind.value}</div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-8">
            ...
            {testimonials.length == -1 && (
              <section>
                <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] mb-6">{ui.investor.testimonialsTitle}</h2>
                <div className="grid sm:grid-cols-2 gap-5">
                  {testimonials.map((t) => (
                    <div key={t.id} className="bg-[#F8F9FA] dark:bg-[#c93239]/30 rounded-2xl p-6 border border-gray-100 dark:border-white/10">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-semibold text-[#c93239] dark:text-white text-sm">{t.name}</div>
            ...
            {docs.length > 0 && (
              <section>
                <h2 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat'] my-6">{ui.investor.documentsTitle}</h2>
                <div className="grid sm:grid-cols-2 gap-3">
                  {docs.map((d) => (
                    <a
                      ...
                      className="flex items-center gap-3 p-4 border border-gray-100 dark:border-white/10 rounded-xl hover:border-[#E8821A] hover:bg-[#E8821A]/5 transition-all group bg-white dark:bg-[#c93239]/20"
                    >
                      <Download size={18} className="text-[#E8821A] shrink-0" aria-hidden="true" />
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-sm text-[#c93239] dark:text-white truncate">{bi(d, "name", locale)}</div>

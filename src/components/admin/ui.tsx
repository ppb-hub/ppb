...
export function Card({ title, actions, children, className = "" }: { title?: React.ReactNode; actions?: React.ReactNode; children: React.ReactNode; className?: string }) {
  return (
    <section className={`bg-white dark:bg-[#c93239]/30 rounded-2xl border border-gray-100 dark:border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.06)] overflow-hidden ${className}`}>
      {(title || actions) && (
        <header className="flex flex-wrap items-center justify-between gap-3 px-5 py-4 border-b border-gray-100 dark:border-white/10">
          <h2 className="font-bold text-[#c93239] dark:text-white font-['Montserrat'] text-base">{title}</h2>
...
export function Btn({
  ...
  const styles = {
    ...
    gold: "bg-[#D4A843] hover:bg-[#e0bc6a] text-[#c93239]",
    danger: "bg-[#E74C3C] hover:bg-[#c0392b] text-white",
    ghost: "bg-transparent hover:bg-gray-100 dark:hover:bg-white/10 text-[#c93239] dark:text-white",
    outline: "border border-gray-200 dark:border-white/20 hover:border-[#c93239] dark:hover:border-white text-[#c93239] dark:text-white",
  }[variant];
  return (
    <button
      {...rest}
...
export function Label({ children, required }: { children: React.ReactNode; required?: boolean }) {
  return (
    <label className="block text-xs font-semibold text-[#c93239] dark:text-white mb-1.5 uppercase tracking-wide">
      {children}
    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center p-0 sm:p-4" role="dialog" aria-modal="true" aria-label={title}>
      ...
      <div className={`relative bg-white dark:bg-[#0a1628] w-full ${wide ? "sm:max-w-3xl" : "sm:max-w-lg"} rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col`}>
        <header className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-white/10">
          <h3 className="font-bold text-[#c93239] dark:text-white font-['Montserrat']">{title}</h3>

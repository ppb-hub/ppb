  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#c93239] dark:text-white font-['Montserrat']">{config.name}</h1>
        </div>
        {config.create && (
          <Btn onClick={openNew}>
                  ...
                  return (
                    <td key={key} className="py-2.5 pr-4 align-middle">
                      {key === "ativo" ? (
                        ...
                        <span className={isTitle ? "font-medium text-[#c93239] dark:text-white" : "text-gray-500 dark:text-white/60"}>
                          {renderCell(v)}
                        </span>
                      )}
                    </td>

...
/**
 * Editor de projeto 1:1 com o schema ProjectCreate/ProjectUpdate:
 * campos próprios + listas aninhadas (objectives, images, documents).
 * Não há endpoint de upload no backend → imagens/documentos por URL.
 */
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-gray-400 font-mono">/pt/projetos/{derivedSlug || "…"}</p>
          </div>
        </div>
      ...
      <section className="bg-white dark:bg-[#0F2B5B]/30 rounded-2xl border border-gray-100 dark:border-white/10 p-5 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <Field label="Capa (URL da imagem)" hint="Sem upload no backend: usar URL ou caminho /uploads/…">
            <input className={inputCls} value={form.cover_image} onChange={(e) => set("cover_image", e.target.value)} placeholder="/uploads/capa-01.jpg" />
          </Field>
...
function NestedList({
  ...
  fields: Array<{ key: string; label: string; type?: "number" }>;
}) {
  return (

...
export default function StatCard({ stat, locale }: { stat: HomeStatOut; locale: Locale }) {
  ...
  return (
    <div
      ref={ref}
      className="bg-white dark:bg-[#0F2B5B]/40 rounded-2xl p-6 shadow-[0_4px_12px_rgba(0,0,0,0.08)] border border-gray-100 dark:border-white/10 flex flex-col items-center text-center hover:shadow-[0_8px_24px_rgba(0,0,0,0.14)] transition-shadow"
    >
      <div className="w-14 h-14 bg-[#0F2B5B]/8 dark:bg-white/10 rounded-xl flex items-center justify-center mb-4">
        <Icon size={28} style={stat.color ? { color: stat.color } : undefined} className="text-[#D4A843]" aria-hidden="true" />
      </div>
      <div className="text-2xl sm:text-3xl font-bold text-[#0F2B5B] dark:text-white font-['Montserrat'] mb-1">
        {display}
        <br />

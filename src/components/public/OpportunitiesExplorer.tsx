  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {activeSector &&
            facetChip(
              activeSector,
              () => setParam("sector", ""),
              "bg-[#c93239]/10 dark:bg-white/10 text-[#c93239] dark:text-white"
            )}
          {activeType &&
            facetChip(
              activeType,
      ...
      {paginated.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {paginated.map((op) => {
            ...
            return (
              ...
              >
                {/* Header com ícone */}
                <div className="relative h-40 bg-gradient-to-br from-[#c93239] to-[#1a3d6e] flex items-center justify-center">
                  <div className="w-20 h-20 bg-white/10 rounded-2xl flex items-center justify-center backdrop-blur-sm">
                    <ApiIcon name={op.icon} size={36} className="text-[#D4A843]" />
                  </div>
                  <div className="absolute top-3 left-3">
                    <span className="bg-[#D4A843] text-[#c93239] text-xs font-bold px-2.5 py-1 rounded-full">
                ...
                <div className="p-5">
                  ...
                  <h3 className="font-bold text-[#c93239] dark:text-white font-['Montserrat'] text-base leading-snug mb-2 line-clamp-2">
                    {bi(op, "area", locale)}
                  </h3>
        ...
        <div className="space-y-3">
          {paginated.map((op) => (
            <div
              key={op.id}
              className="bg-white dark:bg-[#c93239]/30 border border-gray-100 dark:border-white/10 rounded-xl p-4 flex items-center gap-4 hover:shadow-md transition-shadow"
            >
              <div className="relative w-14 h-14 shrink-0 rounded-xl bg-gradient-to-br from-[#c93239] to-[#1a3d6e] flex items-center justify-center">
                <ApiIcon name={op.icon} size={24} className="text-[#D4A843]" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="bg-[#D4A843] text-[#c93239] text-xs font-bold px-2 py-0.5 rounded-full">
                    {ui.investor.opportunityBadge || "Oportunidade"}
                  </span>
                  {op.type && (
                ...
                <h3 className="font-semibold text-[#c93239] dark:text-white text-sm truncate">
                  {bi(op, "area", locale)}
                </h3>
              ...
              <Link
                href={`${localizedHref(locale, "contact")}?interesse=investidor&oportunidade=${op.id}`}
                className="px-4 py-2 bg-[#c93239] hover:bg-[#E8821A] text-white text-xs font-medium rounded-lg transition-colors whitespace-nowrap"
              >
              ...
              .map((n) => (
                <button
                  ...
                  className={cx(
                    ...
                    n === safePage
                      ? "bg-[#c93239] text-white"
                      : "border border-gray-200 dark:border-white/20 text-gray-600 dark:text-white/60 hover:border-[#c93239]"
                  )}
                >
                  {n}
                </button>
        ...
        <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label={ui.projects.filters}>
          ...
          <div className="relative ml-auto w-80 max-w-full h-full bg-white dark:bg-[#c93239] shadow-2xl overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-white/10">
              <h2 className="font-bold text-[#c93239] dark:text-white font-['Montserrat']">
                {ui.projects.filters}
              </h2>
              <button
      ...
      {toastMsg && (
        <div
          role="status"
          className="fixed bottom-20 left-1/2 -translate-x-1/2 bg-[#c93239] text-white text-sm px-5 py-3 rounded-xl shadow-lg flex items-center gap-2 z-50 animate-fade-in"
        >

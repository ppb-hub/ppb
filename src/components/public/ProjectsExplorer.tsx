    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-5 py-3 border border-gray-200 dark:border-white/20 rounded-xl text-sm font-medium text-[#c93239] dark:text-white hover:border-[#c93239] dark:hover:border-white transition-colors bg-white dark:bg-white/5"
        >
        ...
        <select
          ...
          className="px-4 py-3 border border-gray-200 dark:border-white/20 bg-white dark:bg-[#c93239]/30 text-[#1a2332] dark:text-white rounded-xl text-sm focus:outline-none focus:border-[#E8821A]"
        >
        ...
        <div className="flex border border-gray-200 dark:border-white/20 rounded-xl overflow-hidden">
          <button
            ...
            className={cx("px-3 py-3 transition-colors", viewMode === "grid" ? "bg-[#c93239] text-white" : "bg-white dark:bg-white/5 text-gray-500 dark:text-white/60 hover:bg-gray-50")}
          ...
          <button
            ...
            className={cx("px-3 py-3 transition-colors", viewMode === "list" ? "bg-[#c93239] text-white" : "bg-white dark:bg-white/5 text-gray-500 dark:text-white/60 hover:bg-gray-50")}
      ...
      {facetCount > 0 && (
        <div className="flex flex-wrap gap-2 mb-5">
          {activeMunicipality &&
            facetChip(catalogName(municipalities.find((m) => m.slug === activeMunicipality), locale) || activeMunicipality, () => setParam("municipality", ""), "bg-[#c93239]/10 dark:bg-white/10 text-[#c93239] dark:text-white")}
          {activeSector &&
      ...
      {paginated.length === 0 ? (
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {paginated.map((p) => (
            <div
              key={p.id}
              className="group bg-white dark:bg-[#c93239]/30 rounded-2xl overflow-hidden border border-gray-100 dark:border-white/10 shadow-[0_4px_12px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.14)] transition-all duration-300 hover:-translate-y-1"
            >
              ...
              <div className="p-5">
                <span className="text-xs bg-[#c93239]/8 dark:bg-white/10 text-[#c93239] dark:text-white/70 px-2.5 py-1 rounded-full mb-2 inline-block">
                  {catalogName(p.sector, locale)}
                </span>
                <h3 className="font-bold text-[#c93239] dark:text-white font-['Montserrat'] text-sm leading-snug mb-2 line-clamp-2">
                  {bi(p, "title", locale)}
                </h3>
                ...
                <div className="mb-3">
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-semibold text-[#c93239] dark:text-white">{p.progress}%</span>
                  </div>
                ...
                <Link
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-[#c93239] hover:bg-[#E8821A] text-white text-sm font-medium py-2.5 rounded-xl transition-colors duration-200"
                >
        ...
        <div className="space-y-3">
          {paginated.map((p) => (
            <div key={p.id} className="bg-white dark:bg-[#c93239]/30 border border-gray-100 dark:border-white/10 rounded-xl p-4 flex items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                <h3 className="font-semibold text-[#c93239] dark:text-white text-sm truncate">{bi(p, "title", locale)}</h3>
              ...
              <div className="hidden sm:flex items-center gap-4 shrink-0">
                <div className="w-24">
                  <div className="text-xs text-right mb-1 font-medium text-[#c93239] dark:text-white">{p.progress}%</div>
                  <ProgressBar progress={p.progress} />
                </div>
                <Link
                  href={localizedHref(locale, "projects", p.slug)}
                  className="px-4 py-2 bg-[#c93239] hover:bg-[#E8821A] text-white text-xs font-medium rounded-lg transition-colors"
                >
                  {locale === "pt" ? "Ver" : "View"}
                </Link>
              </div>
        ...
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center">
            <button
              ...
              className="w-9 h-9 flex items-center justify-center rounded-lg border border-gray-200 dark:border-white/20 disabled:opacity-40 hover:border-[#c93239] dark:hover:border-white transition-colors"
            >
              <ChevronLeft size={16} />
            </button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).slice(0, 12).map((n) => (
              <button
                ...
                className={cx(
                  ...
                  n === safePage ? "bg-[#c93239] text-white" : "border border-gray-200 dark:border-white/20 text-gray-600 dark:text-white/60 hover:border-[#c93239]"
                )}
              >
                {n}
              </button>
      ...
      {filterOpen && (
        <div className="fixed inset-0 z-50 flex" role="dialog" aria-modal="true" aria-label={ui.projects.filters}>
          ...
          <div className="relative ml-auto w-80 max-w-full h-full bg-white dark:bg-[#c93239] shadow-2xl overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-white/10">
              <h2 className="font-bold text-[#c93239] dark:text-white font-['Montserrat']">{ui.projects.filters}</h2>
      ...
      {toastMsg && (
        <div role="status" className="fixed bottom-20 left-1/2 -translate-x-1/2 bg-[#c93239] text-white text-sm px-5 py-3 rounded-xl shadow-lg flex items-center gap-2 z-50 animate-fade-in">
...
function FacetGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="font-semibold text-sm text-[#c93239] dark:text-white mb-3">{title}</h3>
